...
const MessagesTable = ({ messages: initialMessages }: { messages: ContactMessage[] }) => {
  ...
  const success = Array.isArray(optimisticMessages) || "items" in optimisticMessages;
  ...
  const handleReplyToggle = async (id: string, currentStatus: boolean) => {
    ...
    if ("code" in result) {
      ...
      toast({
        title: "Success",
        description: `Message marked as ${!currentStatus ? "replied" : "pending"}`,
      });
    }
  };
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-black/20">
                ...
                <TableHead className="text-black font-bold">Mark as Replied</TableHead>
              ...
              {displayedMessages.length === 0 ? (
                ...
                displayedMessages.map((message) => (
                  <TableRow key={message.id}>
                    ...
                    <TableCell>{formatToMonthDayYear(message.submissionDate)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={() => handleReplyToggle(message.id, message.replied)}
                      />
                    </TableCell>
                    <TableCell>
                      <div className="flex justify-end gap-2">
                        <PreviewModal content={message.message} author={message.fullName} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))
