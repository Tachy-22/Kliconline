...
const TestimonyTable = ({ testimonies: initialTestimonies }: { testimonies: TestimonyT[] }) => {
  ...
  const handleApprovalToggle = async (id: string, currentStatus: boolean) => {
    ...
    if ("code" in result) {
      ...
      toast({
        title: "Success",
        description: `Testimony marked as ${!currentStatus ? "approved" : "not approved"}`,
      });
    }
  };
  ...
  const success = Array.isArray(initialTestimonies) || "items" in initialTestimonies
    ...
    return (
      <motion.div {...slideIn}>
        <Card className="min-w-full mx-auto !bg-white border-0">
          <CardContent className="flex justify-center p-4">Loading...</CardContent>
        </Card>
      </motion.div>
    )
  }
              ...
              {displayedTestimonies.length === 0 ? (
                ...
                displayedTestimonies.map((testimony) => (
                  <TableRow key={testimony.id} className="border-black/20">
                    ...
                    <TableCell>{formatToMonthDayYear(testimony.date)}</TableCell>
                    <TableCell className="max-w-[300px] truncate">{testimony.content}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={() => handleApprovalToggle(testimony?.id as string, testimony?.approved as boolean)}
                      />
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <PreviewModal content={testimony.content} author={testimony.author} />
                        <EditModal title="Edit Testimony">
                          <TestimonyForm testimony={testimony} update />
                        </EditModal>
